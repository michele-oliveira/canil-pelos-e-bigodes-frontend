    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      try {
        if (setImage) {
          ...
          throw new Error(
            "Either 'setImage' or 'onChange' props must be passed to ProfilePictureSelector"
          );
        }
  ...
  return (
    ...
    >
      <button
        ...
        onDrop={handleDrop}
        className={`relative flex items-center justify-center w-56 h-56 overflow-hidden border-2 ${
      ...
      >
        ...
        <div className="absolute bottom-0 flex items-center justify-center w-full py-1 text-3xl text-white bg-slate-800">
          <UploadIcon />
        </div>
      </button>
    </div>
